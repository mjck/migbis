...
#include "vtkmpjImageROISource.h"
...
vtkStandardNewMacro(vtkmpjImageROISource);

vtkmpjImageROISource::vtkmpjImageROISource()
{
  this->Density = 1.0;
  ...
  this->SourceType=0;   // bound to mask
  this->Radius = 1.0;
...
void vtkmpjImageROISource::Execute()
{
  double x1, y1, z1;
  double x[3];
  int i, j, k;
  ...
  if (this->SourceType == 0 && this->Mask != NULL) 
  {
    //int dim[3];
    //this->Mask->GetDimensions(dim);
    //mask = this->Mask->GetPointData()->GetScalars();
    ...
    // spherical mode
    //printf("in sphere mode\n");
    xmin = this->Center[0] - this->Radius;
    ymin = this->Center[1] - this->Radius;
    zmin = this->Center[2] - this->Radius;
    xmax = this->Center[0] + this->Radius;
    ymax = this->Center[1] + this->Radius;
    zmax = this->Center[2] + this->Radius;
    
    for(x1 = xmin; x1 <= xmax; x1 += incx) 
      for(y1 = ymin; y1 <= ymax; y1 += incy)
        for(z1 = zmin; z1 <= zmax; z1 += incz) 
        {
           if ((x1-this->Center[0])*(x1-this->Center[0]) +
               (y1-this->Center[1])*(y1-this->Center[1]) +
               (z1-this->Center[2])*(z1-this->Center[2]) <= this->Radius*this->Radius)
               {
                  x[0] = x1; 
                  x[1] = y1;
                  x[2] = z1;
