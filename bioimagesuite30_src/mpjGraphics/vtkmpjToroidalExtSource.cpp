...
vtkStandardNewMacro(vtkmpjToroidalExtSource);

//----------------------------------------------------------------------------
void vtkmpjToroidalExtSource::Evaluate(double theta, double phi,  
                             double dtheta, double dphi,                       
                             double dims[3],                            
                             double xyz[3],      
                             double nrm[3])     
{
  double cu = cos(phi + vtkMath::Pi() / 2.0);
  double su = sin(phi + vtkMath::Pi() / 2.0);
  double cv = cos(theta);
  double sv = sin(theta);
  ...
  xyz[0] = dims[2] * (this->Alpha * cu + this->Beta * cv * cu);
  xyz[1] = dims[1] * (this->Alpha * su + this->Beta * cv * su);
  xyz[2] = dims[0] * sv;
...
vtkmpjToroidalExtSource::vtkmpjToroidalExtSource(int res)
{
  res = res < 4 ? 4 : res;
...
void vtkmpjToroidalExtSource::SetPhiResolution(int i)
{
  if(i < 4)
    {
    i = 4;
...
void vtkmpjToroidalExtSource::SetThetaResolution(int i)
{
  if(i < 8)
    {
    i = 8;
...
void vtkmpjToroidalExtSource::Execute()
{
...
void vtkmpjToroidalExtSource::PrintSelf(ostream& os, vtkIndent indent)
{
