  ...
  if ( inScalars )
    {
      ...
      //printf("range0: %f, %f; range2: %f, %f\n",range0[0],range0[1],range2[0],range2[1]);
      if ((range2[1] - range0[0]) == 0.0)
	{
                                  ...
                                  vtkDataArray *inNormals)
{
  ...
  for (j=0; j < npts; j++)
	...
	sFactor[2] = 1.0 +  
      ...
      if (sFactor[0] < 0.0 || sFactor[1] < 0.0 || sFactor[2]) 
        {
    ...
    /*printf("Normal: %d, Binormal: %d, NormalScale=%f, BinormalScale=%f, sFactors=%f,%f,%f, real=%f,%f,%f\n",
	   this->NormalEigenvector,this->BinormalEigenvector,this->NormalScale,this->BinormalScale,
	   sFactor[0],sFactor[1],sFactor[2],
	   inScalars->GetComponent(pts[j],0),inScalars->GetComponent(pts[j],1),inScalars->GetComponent(pts[j],2) );*/
    ...
    if (this->SidesShareVertices)
      ...
      for (k=0; k < this->NumberOfSides; k++)
        ...
        for (i=0; i<3; i++) 
          {
          normal[i] = w[i]*cos((double)k*this->Theta)*sFactorW + 
            nP[i]*sin((double)k*this->Theta)*sFactorNP;
      ...
      for (k=0; k < this->NumberOfSides; k++)
        ...
        for (i=0; i<3; i++)
          ...
          // tube as it bends.
          normal[i]  = w[i]*cos((double)(k+0.0)*this->Theta)*sFactorW + 
            nP[i]*sin((double)(k+0.0)*this->Theta)*sFactorNP;
          n_right[i] = w[i]*cos((double)(k-0.5)*this->Theta)*sFactorW + 
            nP[i]*sin((double)(k-0.5)*this->Theta)*sFactorNP;
          n_left[i]  = w[i]*cos((double)(k+0.5)*this->Theta)*sFactorW + 
            nP[i]*sin((double)(k+0.5)*this->Theta)*sFactorNP;
          s[i] = p[i] + this->Radius * normal[i];
          }
        newPts->InsertPoint(ptId,s);
