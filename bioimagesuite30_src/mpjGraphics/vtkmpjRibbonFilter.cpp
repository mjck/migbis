...
vtkStandardNewMacro(vtkmpjRibbonFilter);

// Construct object with radius 0.5, radius variation turned off, the number 
// of sides set to 3, and radius factor of 10.
vtkmpjRibbonFilter::vtkmpjRibbonFilter()
{
  this->Radius = 0.5;
  this->VaryRadius = VTK_VARY_RADIUS_OFF;
  this->NumberOfSides = 8;   // 4
  ...
  this->BinormalEigenvector = 2;
  this->NormalScale = 1.0;
  this->BinormalScale = 1.0;
}

void vtkmpjRibbonFilter::Execute()
{
  ...
  vtkIdType i;
  double range0[2], range2[2], maxSpeed=0;
  ...
  newNormals->SetName("RibbonNormals");
  ...
  if ( inScalars )
    {
      inScalars->GetRange(range0,0);  // range of lambda1
      inScalars->GetRange(range2,2);  // range of lambda3
      if ((range0[1] - range2[0]) == 0.0)
	{
	  if (this->VaryRadius == VTK_VARY_RADIUS_BY_SCALAR )
	    {
	      vtkWarningMacro(<< "Scalar range is zero!");
	    }
	  range0[1] = range0[0] + 1.0;
	}
      range0[0] = range2[0];
  ...
  for (inCellId=0, inLines->InitTraversal(); 
    ...
    if ( !this->GeneratePoints(offset,npts,pts,inPts,newPts,pd,outPD,
                               newNormals,inScalars,range0,inVectors,
                               maxSpeed,inNormals) )
      {
...
int vtkmpjRibbonFilter::GeneratePoints(vtkIdType offset, 
                                  ...
                                  vtkDataArray *inNormals)
{
  ...
  for (j=0; j < npts; j++)
    ...
    if ( inScalars && this->VaryRadius == VTK_VARY_RADIUS_BY_SCALAR )
      {
	/*sFactor[0] = 1.0 + ((this->RadiusFactor - 1.0) * 
			    (inScalars->GetComponent(pts[j],0) - range[0]) 
			    / (range[1]-range[0]));
	sFactor[1] = 1.0 + ((this->RadiusFactor - 1.0) * 
			    (inScalars->GetComponent(pts[j],1) - range[0]) 
			    / (range[1]-range[0]));
	sFactor[2] = 1.0 + ((this->RadiusFactor - 1.0) * 
			    (inScalars->GetComponent(pts[j],2) - range[0]) 
			    / (range[1]-range[0]));*/
	sFactor[0] =  1.0 +
			    (inScalars->GetComponent(pts[j],0) - range[0]) 
			    / (range[1]-range[0]);
	...
	sFactor[2] =  1.0 +
      ...
      if ( sFactor[0] > this->RadiusFactor )
        {
        sFactor[0] = this->RadiusFactor;
        }
      sFactor[2] = sFactor[1] = sFactor[0];
      }
    else if ( inScalars && 
              this->VaryRadius == VTK_VARY_RADIUS_BY_ABSOLUTE_SCALAR )
      {
      sFactor[0] = inScalars->GetComponent(pts[j],0);
      sFactor[1] = inScalars->GetComponent(pts[j],1);
      sFactor[2] = inScalars->GetComponent(pts[j],2);
      if (sFactor[0] < 0.0 || sFactor[1] < 0.0 || sFactor[2] < 0.0) 
        {
    ...
    double sFactorNP = sFactor[this->NormalEigenvector] * this->NormalScale;
    double sFactorW = sFactor[this->BinormalEigenvector] * this->BinormalScale;
    
    if (this->VaryRadius == VTK_VARY_RADIUS_OFF)
      {
	sFactorNP = this->NormalScale;
	sFactorW = this->BinormalScale;
      }
	...
	/*for (k=0; k < this->NumberOfSides; k++)
	 ...
	 for (i=0; i<3; i++) 
	   {
	     normal[i]  = w[i]*sFactorW;
	     s[i] = p[i] + this->Radius * normal[i];
	   }
        newPts->InsertPoint(ptId,s);
        newNormals->InsertTuple(ptId,normal);
        outPD->CopyData(pd,pts[j],ptId);
	 ...
	 for (i=0; i<3; i++) 
	   {
	     normal[i]  = w[i]*sFactorW + nP[i]*sFactorNP;
	     s[i] = p[i] + this->Radius * normal[i];
	   }
	...
	float nl[3], nr[3];
      /*for (k=0; k < this->NumberOfSides; k++)
        ...
        for (i=0; i<3; i++)
          ...
          n_left[i]  = w[i]*cos((double)(k+0.5)*this->Theta) + 
            nP[i]*sin((double)(k+0.5)*this->Theta);
          s[i] = p[i] + this->Radius * sFactor[0] * normal[i];
          }
        newPts->InsertPoint(ptId,s);
	 ...
	 outPD->CopyData(pd,pts[j],ptId);
	 newPts->InsertPoint(ptId+1,s);
	 newNormals->InsertTuple(ptId+1,normal);
	 outPD->CopyData(pd,pts[j],ptId+1);
	 ptId += 2;
	 ...
	 newPts->InsertPoint(ptId,s);
	 newNormals->InsertTuple(ptId,nl);
	 outPD->CopyData(pd,pts[j],ptId);
	 newPts->InsertPoint(ptId+1,s);
	 newNormals->InsertTuple(ptId+1,nr);
	 outPD->CopyData(pd,pts[j],ptId+1);
	 ptId += 2;
...
void vtkmpjRibbonFilter::GenerateStrips(vtkIdType offset, vtkIdType npts, 
...
void vtkmpjRibbonFilter::GenerateTextureCoords(vtkIdType offset,
                                          vtkIdType npts, vtkIdType *pts, 
                                          vtkPoints *inPts, 
                                          ...
                                          vtkFloatArray *newTCoords)
{
  ...
  if ( this->GenerateTCoords == VTK_TCOORDS_FROM_SCALARS )
    ...
    for (i=1; i < npts; i++)
      ...
      for ( k=0; k < numSides; k++)
        {
        newTCoords->InsertTuple2(offset+i*numSides+k,tc,(double)k/(numSides-1));
        }
      xPrev[0]=x[0]; xPrev[1]=x[1]; xPrev[2]=x[2];
      }
    }
  ...
  if ( this->Capping )
    ...
    for (ik=0; ik < this->NumberOfSides; ik++)
      {
      newTCoords->InsertTuple2(startIdx+ik,0.0,(double)ik/(this->NumberOfSides-1));
      }

    //end cap
    for (ik=0; ik < this->NumberOfSides; ik++)
      {
      newTCoords->InsertTuple2(startIdx+this->NumberOfSides+ik,tc,(double)ik/(this->NumberOfSides-1));
      }
    }
}

// Compute the number of points in this tube
vtkIdType vtkmpjRibbonFilter::ComputeOffset(vtkIdType offset, vtkIdType npts)
{
  if ( this->SidesShareVertices )
    {
...
const char *vtkmpjRibbonFilter::GetVaryRadiusAsString(void)
{
  if ( this->VaryRadius == VTK_VARY_RADIUS_OFF )
    {
...
const char *vtkmpjRibbonFilter::GetGenerateTCoordsAsString(void)
{
...
void vtkmpjRibbonFilter::PrintSelf(ostream& os, vtkIndent indent)
{
