...
vtkStandardNewMacro(vtkmpjSuperToroidalExtSource);

//----------------------------------------------------------------------------
double vtkmpjSuperToroidalExtSource::Power ( double x, double n )
{
  if ( x == 0 )
    {
    return 0;
...
//----------------------------------------------------------------------------
void vtkmpjSuperToroidalExtSource::Evaluate(double theta, double phi,  
											...
											double nrm[3])     
{
  ...
  double sxi = this->Power( sin( vtkMath::Pi() * this->Alpha ), this->Xi );
  double mxi = 1.0 - this->Power( sin( vtkMath::Pi() * this->Beta ), this->Xi);

  if (this->NewSuperToroid) {
    sxi = this->Alpha;
    mxi = this->Beta;
  }

  double cun1 = this->Power(cu,this->N1);
  double cvn2 = this->Power(cv,this->N2);
  double sun1 = this->Power(su,this->N1);
  double svn2 = this->Power(sv,this->N2);
  ...
  if (this->SwapAxes == 0) {
    ...
    // super toroid gen #2
    xyz[0] = dims[2] * (sxi * cun1 + mxi * cvn2 * cun1);
    xyz[1] = dims[1] * (sxi * sun1 + mxi * cvn2 * sun1);
    xyz[2] = dims[0] * svn2; 
    ...
    xyz[0] = dims[2] * svn2;	
    xyz[1] = dims[1] * (sxi * sun1 + mxi * cvn2 * sun1);
    xyz[2] = dims[0] * (sxi * cun1 + mxi * cvn2 * cun1);	
...
vtkmpjSuperToroidalExtSource::vtkmpjSuperToroidalExtSource(int res)
{
  res = res < 4 ? 4 : res;
...
void vtkmpjSuperToroidalExtSource::SetPhiResolution(int i)
{
  if(i < 4)
    {
    i = 4;
...
void vtkmpjSuperToroidalExtSource::SetThetaResolution(int i)
{
  if(i < 8)
    {
    i = 8;
...
void vtkmpjSuperToroidalExtSource::Execute()
{
  // Get the output
...
void vtkmpjSuperToroidalExtSource::PrintSelf(ostream& os, vtkIndent indent)
{
